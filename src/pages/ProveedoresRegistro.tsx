...
import { getProveedores, deleteProveedor, createProveedor, updateProveedor } from '../services/proveedorService';
...
const ProveedoresRegistro = () => {
    ...
    useEffect(() => {
        ...
        const filtered = proveedores.filter(p =>
            p.proveedor.toLowerCase().includes(lowerSearchTerm) ||
            p.nit.toLowerCase().includes(lowerSearchTerm) ||
            (p.contacto && p.contacto.toLowerCase().includes(lowerSearchTerm))
        );
    ...
    const fetchProveedores = async () => {
        try {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: `Error al cargar proveedores: ${err.message}`
            });
        } finally {
            setLoading(false);
        }
    ...
    const handleReactivate = async (id: string) => {
        ...
        if (result.isConfirmed) {
            try {
                await updateProveedor(id, { estado: 'activo' });
                setProveedores(proveedores.map(p =>
    ...
    const exportToExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(filteredProveedores.map(p => ({
    ...
    const exportToPDF = () => {
        ...
        doc.text('Registro de Proveedores', 14, 22);
        ...
        autoTable(doc, {
            head: [['Proveedor', 'NIT', 'Contacto', 'Celular', 'Dirección', 'Estado']],
            body: filteredProveedores.map(p => [
                p.proveedor,
                p.nit,
                p.contacto || '',
                p.celular || '',
    ...
    const manualSections: ManualSection[] = [
        {
            ...
            content: 'Módulo para registrar y administrar la información de los proveedores de la empresa.'
        },
        {
            title: 'Crear Proveedor',
            content: 'Utilice el botón "Nuevo Proveedor" para registrar una empresa o persona que suministra materiales.'
        }
    ];
    ...
    return (
        <div className="container mx-auto relative min-h-screen">
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 no-print">
                <div className="flex items-center gap-4">
                    <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Gestiona el directorio de proveedores y sus contactos</p>
                    </div>
                </div>
            ...
            <div className="mb-2 text-sm text-gray-500 dark:text-gray-400">
                Mostrando {filteredProveedores.length === 0 ? 0 : indexOfFirstItem + 1} - {Math.min(indexOfLastItem, filteredProveedores.length)} de {filteredProveedores.length} registros
            </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                                ...
                                <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Proveedor</th>
                                <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">NIT</th>
                                <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Contacto / Celular</th>
                                <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Dirección</th>
                            ...
                            {currentItems.map((p, index) => (
                                <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td className="p-4 text-gray-700 dark:text-gray-200 font-medium">
                                        {p.proveedor}
                                        {p.email && <div className="text-xs text-gray-500 hover:text-blue-500 mt-1">{p.email}</div>}
                                    </td>
                                    ...
                                    <td className="p-4 text-gray-700 dark:text-gray-200">
                                        <div className="flex flex-col">
                                            {p.contacto && <span className="font-semibold text-sm">{p.contacto}</span>}
                                            {p.celular && (
                                    ...
                                    <td className="p-4 text-gray-500 dark:text-gray-400 text-sm max-w-xs truncate" title={p.direccion}>
                                        {p.direccion}
                                        {p.telefono && <div className="text-xs mt-1 text-gray-400">Tel: {p.telefono}</div>}
                                    </td>
                                    <td className="p-4">
                ...
                <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4 backdrop-blur-sm">
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden transform transition-all scale-100">
                        <ProveedoresForm
                            ...
                            onSubmit={handleSave}
                            onCancel={() => { setShowModal(false); setEditingProveedor(null); }}
                        />
                    </div>
                </div>
            )}
