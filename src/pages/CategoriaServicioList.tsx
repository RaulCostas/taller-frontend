...
import { getCategoriasServicio, deleteCategoriaServicio, createCategoriaServicio, updateCategoriaServicio } from '../services/categoriaServicioService';
import { CategoriaServicioForm } from '../components/categoriaServicio/CategoriaServicioForm';
...
const CategoriaServicioList = () => {
    const navigate = useNavigate();
    const [categorias, setCategorias] = useState<CategoriaServicio[]>([]);
    const [filteredCategorias, setFilteredCategorias] = useState<CategoriaServicio[]>([]);
    ...
    useEffect(() => {
        ...
        const filtered = categorias.filter(c =>
            c.categoria_servicio.toLowerCase().includes(lowerSearchTerm)
        );
    ...
    const fetchCategorias = async () => {
        try {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: `Error al cargar categorías: ${err.message || 'Error desconocido'}`
            });
        } finally {
            setLoading(false);
        }
    ...
    const handleReactivate = async (id: string) => {
        ...
        if (result.isConfirmed) {
            try {
                await updateCategoriaServicio(id, { estado: 'activo' });
                setCategorias(categorias.map(c =>
    ...
    const handleSave = async (categoriaData: Omit<CategoriaServicio, 'id'>) => {
        try {
            if (editingCategoria) {
                await updateCategoriaServicio(editingCategoria.id, categoriaData);
                await Swal.fire({
                    icon: 'success',
                    title: 'Actualizado',
                ...
                await createCategoriaServicio(categoriaData);
                await Swal.fire({
                    icon: 'success',
                    title: 'Creado',
    ...
    const openEditModal = (categoria: CategoriaServicio) => {
        setEditingCategoria(categoria);
        setShowModal(true);
    };

    const exportToExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(filteredCategorias.map(c => ({
            'Categoría': c.categoria_servicio,
            'Estado': c.estado
        })));
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "Categorías");
        XLSX.writeFile(workbook, `CategoriasServicio_${new Date().toISOString().split('T')[0]}.xlsx`);
    };
    ...
    const exportToPDF = () => {
        ...
        doc.text('Reporte de Categorías de Servicios', 14, 22);
        ...
        autoTable(doc, {
            ...
            body: filteredCategorias.map(c => [c.categoria_servicio, c.estado]),
            startY: 30,
            headStyles: { fillColor: [52, 152, 219] },
            alternateRowStyles: { fillColor: [240, 248, 255] },
        });

        doc.save(`CategoriasServicio_${new Date().toISOString().split('T')[0]}.pdf`);
    };
    ...
    const manualSections: ManualSection[] = [
        {
            ...
            content: 'Aquí puede administrar las categorías de servicios del taller.'
        },
        {
            title: 'Crear Categoría',
            content: 'Haga clic en "+ Nueva Categoría", ingrese el nombre y guarde.'
        },
        {
            title: 'Desactivar/Reactivar',
            content: 'Use el botón rojo para desactivar y el verde para reactivar.'
        }
    ];
    ...
    return (
        <div className="container mx-auto relative min-h-screen">
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 no-print">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Gestiona las categorías de los servicios ofrecidos</p>
                    </div>
                </div>
            ...
            <div className="mb-2 text-sm text-gray-500 dark:text-gray-400">
                Mostrando {filteredCategorias.length === 0 ? 0 : indexOfFirstItem + 1} - {Math.min(indexOfLastItem, filteredCategorias.length)} de {filteredCategorias.length} registros
            </div>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {currentItems.map((c, index) => (
                                <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td className="p-4 text-gray-700 dark:text-gray-200 font-medium">{c.categoria_servicio}</td>
                                    <td className="p-4">
                                        <span className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wide border ${c.estado === 'activo'
                ...
                <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4 backdrop-blur-sm">
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100">
                        <CategoriaServicioForm
                            ...
                            onSubmit={handleSave}
                            onCancel={() => { setShowModal(false); setEditingCategoria(null); }}
                        />
                    </div>
                </div>
            )}
...
export default CategoriaServicioList;
