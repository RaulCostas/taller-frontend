...
import type { AreaPersonal, CreateAreaPersonalData } from '../types/areaPersonal';
import { getAreasPersonal, deleteAreaPersonal, createAreaPersonal, updateAreaPersonal } from '../services/areaPersonalService';
...
import { AreaPersonalForm } from '../components/areaPersonal/AreaPersonalForm';
import { ArrowLeft, Tags } from 'lucide-react';

const AreaPersonalList = () => {
    const navigate = useNavigate();
    const [areas, setAreas] = useState<AreaPersonal[]>([]);
    const [filteredAreas, setFilteredAreas] = useState<AreaPersonal[]>([]);
    const [loading, setLoading] = useState(true);
    const [searchTerm, setSearchTerm] = useState('');
    const [showModal, setShowModal] = useState(false);
    const [editingArea, setEditingArea] = useState<AreaPersonal | null>(null);
    ...
    useEffect(() => {
        ...
        const filtered = areas.filter(a =>
            a.area.toLowerCase().includes(lowerSearchTerm)
        );
    ...
    const fetchAreas = async () => {
        try {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: `Error al cargar áreas: ${err.message}`
            });
        } finally {
            setLoading(false);
        }
    ...
    const handleDelete = async (id: string) => {
        ...
        if (result.isConfirmed) {
            try {
                await deleteAreaPersonal(id);
                setAreas(areas.map(a =>
                    a.id === id ? { ...a, estado: 'inactivo' } : a
                ));
                await Swal.fire({
                    icon: 'success',
                    title: '¡Desactivado!',
    ...
    const handleReactivate = async (id: string) => {
        ...
        if (result.isConfirmed) {
            try {
                await updateAreaPersonal(id, { estado: 'activo' });
                setAreas(areas.map(a =>
                    a.id === id ? { ...a, estado: 'activo' } : a
                ));
                await Swal.fire({
                    icon: 'success',
                    title: '¡Reactivada!',
    ...
    const openEditModal = (area: AreaPersonal) => {
        setEditingArea(area);
        setShowModal(true);
    };

    const handleSave = async (formData: CreateAreaPersonalData) => {
        try {
            if (editingArea) {
                await updateAreaPersonal(editingArea.id, formData);
                await Swal.fire({
                    icon: 'success',
                    title: 'Actualizado',
                ...
                await createAreaPersonal(formData);
                await Swal.fire({
                    icon: 'success',
                    title: 'Creada',
    ...
    const exportToExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(filteredAreas.map(a => ({
            'Área': a.area,
            'Estado': a.estado
        })));
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "AreasPersonal");
        XLSX.writeFile(workbook, `AreasPersonal_${new Date().toISOString().split('T')[0]}.xlsx`);
    };
    ...
    const exportToPDF = () => {
        ...
        doc.text('Reporte de Áreas de Personal', 14, 22);
        ...
        autoTable(doc, {
            head: [['Área', 'Estado']],
            body: filteredAreas.map(a => [a.area, a.estado]),
            startY: 30,
    ...
    const manualSections: ManualSection[] = [
        {
            ...
            content: 'Aquí puede administrar las áreas de personal (Administración, Ventas, etc.).'
        },
        {
            title: 'Crear Área',
            content: 'Haga clic en "+ Nueva Área", ingrese el nombre y guarde.'
        },
        {
    ...
    return (
        <div className="container mx-auto relative min-h-screen">
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 no-print">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Administra las áreas y departamentos del personal</p>
                    </div>
                </div>
            ...
            <div className="mb-2 text-sm text-gray-500 dark:text-gray-400">
                Mostrando {filteredAreas.length === 0 ? 0 : indexOfFirstItem + 1} - {Math.min(indexOfLastItem, filteredAreas.length)} de {filteredAreas.length} registros
            </div>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {currentItems.map((area, index) => (
                                <tr key={area.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td className="p-4">
                                        <span className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wide border ${area.estado === 'activo'
            ...
            <ManualModal
                ...
                title="Manual de Áreas de Personal"
                ...
                <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4 backdrop-blur-sm">
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100">
                        <AreaPersonalForm
                            ...
                            onSubmit={handleSave}
                            onCancel={() => { setShowModal(false); setEditingArea(null); }}
                        />
                    </div>
                </div>
            )}
...
export default AreaPersonalList;
