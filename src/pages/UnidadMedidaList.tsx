...
import { getUnidadesMedida, deleteUnidadMedida, createUnidadMedida, updateUnidadMedida } from '../services/unidadMedidaService';
...
const UnidadMedidaList = () => {
    ...
    const [filteredUnidadesMedida, setFilteredUnidadesMedida] = useState<UnidadMedida[]>([]);
    ...
    useEffect(() => {
        ...
        const filtered = unidadesMedida.filter(um =>
            um.medida.toLowerCase().includes(lowerSearchTerm)
        );
    ...
    const fetchUnidadesMedida = async () => {
        try {
            ...
            setFilteredUnidadesMedida(data);
        } catch (err: any) {
            console.error(err);
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: `Error al cargar unidades de medida: ${err.message || 'Error desconocido'}`
            });
        } finally {
            setLoading(false);
        }
    ...
    const currentItems = filteredUnidadesMedida.slice(indexOfFirstItem, indexOfLastItem);
    ...
    const handleDelete = async (id: string) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                setUnidadesMedida(unidadesMedida.map(um =>
                    um.id === id ? { ...um, estado: 'inactivo' } : um
                ));
                await Swal.fire({
                    icon: 'success',
                    title: '¡Desactivado!',
    ...
    const handleReactivate = async (id: string) => {
        ...
        if (result.isConfirmed) {
            try {
                await updateUnidadMedida(id, { estado: 'activo' });
                setUnidadesMedida(unidadesMedida.map(um =>
                    um.id === id ? { ...um, estado: 'activo' } : um
                ));
                await Swal.fire({
                    icon: 'success',
                    title: '¡Reactivado!',
    ...
    const handleSave = async (unidadMedidaData: CreateUnidadMedidaData) => {
        try {
            if (editingUnidadMedida) {
                await updateUnidadMedida(editingUnidadMedida.id, unidadMedidaData);
                await Swal.fire({
                    icon: 'success',
                    title: 'Actualizado',
                ...
                await createUnidadMedida(unidadMedidaData);
                await Swal.fire({
                    icon: 'success',
                    title: 'Creado',
    ...
    const openEditModal = (unidadMedida: UnidadMedida) => {
        setEditingUnidadMedida(unidadMedida);
        setShowModal(true);
    };

    const exportToExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(filteredUnidadesMedida.map(um => ({
            'Medida': um.medida,
            'Estado': um.estado
        })));
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "Unidades de Medida");
        XLSX.writeFile(workbook, `UnidadesMedida_${new Date().toISOString().split('T')[0]}.xlsx`);
    };
    ...
    const exportToPDF = () => {
        ...
        autoTable(doc, {
            head: [['Medida', 'Estado']],
            body: filteredUnidadesMedida.map(um => [um.medida, um.estado]),
            startY: 30,
            headStyles: { fillColor: [52, 152, 219] },
            alternateRowStyles: { fillColor: [240, 248, 255] },
        });

        doc.save(`UnidadesMedida_${new Date().toISOString().split('T')[0]}.pdf`);
    };
    ...
    const manualSections: ManualSection[] = [
        {
            ...
            content: 'Aquí puede administrar las unidades de medida del sistema (Metro, Kilogramo, Litro, Pieza, etc.).'
        },
        {
            title: 'Crear Unidad de Medida',
            content: 'Haga clic en "+ Nueva Unidad de Medida", ingrese el nombre y guarde.'
        },
        {
    ...
    return (
        <div className="container mx-auto relative min-h-screen">
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 no-print">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Administra las unidades de medida (kg, m, lts)</p>
                    </div>
                </div>
            ...
            <div className="mb-2 text-sm text-gray-500 dark:text-gray-400">
                Mostrando {filteredUnidadesMedida.length === 0 ? 0 : indexOfFirstItem + 1} - {Math.min(indexOfLastItem, filteredUnidadesMedida.length)} de {filteredUnidadesMedida.length} registros
            </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                                ...
                                <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Unidad de Medida</th>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {currentItems.map((um, index) => (
                                <tr key={um.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    <td className="p-4 text-gray-500 dark:text-gray-400 font-mono text-sm">
                                        {indexOfFirstItem + index + 1}
                                    </td>
                                    <td className="p-4 text-gray-700 dark:text-gray-200 font-medium">{um.medida}</td>
                                    <td className="p-4">
                                        <span className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wide border ${um.estado === 'activo'
                                    ...
                                    <td className="p-4 flex gap-2 justify-end no-print">
                                        ...
                                        {um.estado === 'activo' ? (
                                            ...
                                            <button
                                                onClick={() => handleReactivate(um.id)}
            ...
            <ManualModal
                ...
                title="Manual de Unidades de Medida"
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4 backdrop-blur-sm">
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100">
                        <UnidadMedidaForm
                            initialData={editingUnidadMedida}
                            onSubmit={handleSave}
                            onCancel={() => { setShowModal(false); setEditingUnidadMedida(null); }}
                        />
                    </div>
                </div>
            )}
