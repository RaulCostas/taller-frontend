...
import { getSeguros, deleteSeguro, createSeguro, updateSeguro } from '../services/seguroService';
import { SegurosForm } from '../components/seguros/SegurosForm';
...
const SegurosList = () => {
    ...
    useEffect(() => {
        ...
        const filtered = seguros.filter(s =>
            s.seguro.toLowerCase().includes(lowerSearchTerm) ||
            s.nit.toLowerCase().includes(lowerSearchTerm)
        );
    ...
    const fetchSeguros = async () => {
        try {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: `Error al cargar seguros: ${err.message}`
            });
        } finally {
            setLoading(false);
        }
    ...
    const handleDelete = async (id: string) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                setSeguros(seguros.map(s =>
                    s.id === id ? { ...s, estado: 'inactivo' } : s
                ));
                await Swal.fire({
                    icon: 'success',
                    title: '¡Desactivado!',
                    text: 'Seguro desactivado exitosamente.',
    ...
    const handleReactivate = async (id: string) => {
        ...
        if (result.isConfirmed) {
            try {
                await updateSeguro(id, { estado: 'activo' });
                setSeguros(seguros.map(s =>
                    s.id === id ? { ...s, estado: 'activo' } : s
                ));
                await Swal.fire({
                    icon: 'success',
                    title: '¡Reactivado!',
                    text: 'Seguro reactivado exitosamente.',
                    showConfirmButton: false,
                    timer: 1500
                });
            } catch (error) {
    ...
    const handleSave = async (seguroData: Omit<Seguro, 'id'>) => {
        try {
            if (editingSeguro) {
                await updateSeguro(editingSeguro.id, seguroData);
                await Swal.fire({
                    icon: 'success',
                    title: 'Actualizado',
    ...
    const openEditModal = (seguro: Seguro) => {
        setEditingSeguro(seguro);
        setShowModal(true);
    };

    const exportToExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(filteredSeguros.map(s => ({
            'Seguro': s.seguro,
            'NIT': s.nit,
            'Teléfono': s.telefono,
        ...
        XLSX.writeFile(workbook, `Seguros_${new Date().toISOString().split('T')[0]}.xlsx`);
    };
    ...
    const exportToPDF = () => {
        ...
        doc.text('Reporte de Seguros', 14, 22);
        ...
        autoTable(doc, {
            head: [['Seguro', 'NIT', 'Teléfono', 'Celular', 'Estado']],
            body: filteredSeguros.map(s => [
                s.seguro,
                s.nit,
                s.telefono || '',
                s.celular || '',
        ...
        doc.save(`Seguros_${new Date().toISOString().split('T')[0]}.pdf`);
    };
    ...
    const manualSections: ManualSection[] = [
        {
            ...
            content: 'Modulo para administrar compañías aseguradoras y sus datos de contacto.'
        },
        {
            title: 'Crear',
            content: 'Clic en Nuevo Seguro, llene NIT, Nombre y datos de contacto.'
        },
        {
            title: 'Estados',
            content: 'Use los botones de acción para desactivar (rojo) o reactivar (verde) los seguros.'
        }
    ];
        ...
        <div className="container mx-auto relative min-h-screen">
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 no-print">
                <div className="flex items-center gap-4">
                    <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Gestiona las compañías de seguros y sus datos</p>
                    </div>
                </div>
            ...
            <div className="mb-6 flex items-center gap-2 no-print">
                <div className="relative">
                    ...
                    <input
                        type="text"
                        placeholder="Buscar por seguro o NIT..."
            ...
            <div className="mb-2 text-sm text-gray-500 dark:text-gray-400">
                Mostrando {filteredSeguros.length === 0 ? 0 : indexOfFirstItem + 1} - {Math.min(indexOfLastItem, filteredSeguros.length)} de {filteredSeguros.length} registros
            </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                                ...
                                <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">NIT</th>
                                <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Teléfono/Cel</th>
                                ...
                                <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td className="p-4 text-gray-700 dark:text-gray-200 font-medium">
                                        <div className="flex flex-col">
                                            <span>{s.seguro}</span>
                                            {s.correo && <span className="text-xs text-gray-500">{s.correo}</span>}
                                        </div>
                                    </td>
                                    <td className="p-4 text-gray-600 dark:text-gray-300">{s.nit}</td>
                                    <td className="p-4 text-gray-600 dark:text-gray-300">
                                        <div className="flex flex-col text-sm">
                                            {s.telefono && <span>T: {s.telefono}</span>}
                                            {s.celular && <span>C: {s.celular}</span>}
                                        </div>
                                    </td>
                                    <td className="p-4">
                                        <span className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wide border ${s.estado === 'activo'
                                    ...
                                    <td colSpan={6} className="p-8 text-center text-gray-500 dark:text-gray-400">
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <p className="font-medium">
                                                {searchTerm ? 'No se encontraron resultados' : 'No hay seguros registrados'}
                                            </p>
                                        </div>
                                    </td>
                                </tr>
                ...
                <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4 backdrop-blur-sm">
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden transform transition-all scale-100">
                        <SegurosForm
                            ...
                            onSubmit={handleSave}
                            onCancel={() => { setShowModal(false); setEditingSeguro(null); }}
                        />
                    </div>
                </div>
            )}
