...
import { getFormasPago, deleteFormaPago, createFormaPago, updateFormaPago } from '../services/formaPagoService';
...
const FormaPagoList = () => {
    ...
    const [filteredFormasPago, setFilteredFormasPago] = useState<FormaPago[]>([]);
    ...
    useEffect(() => {
        ...
        const filtered = formasPago.filter(fp =>
            fp.forma_pago.toLowerCase().includes(lowerSearchTerm)
        );
        setFilteredFormasPago(filtered);
        setCurrentPage(1);
    }, [searchTerm, formasPago]);
    ...
    const fetchFormasPago = async () => {
        try {
            ...
            setFormasPago(data);
            setFilteredFormasPago(data);
        } catch (err: any) {
            console.error(err);
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: `Error al cargar formas de pago: ${err.message || 'Error desconocido'}`
            });
        } finally {
            setLoading(false);
        }
    ...
    const currentItems = filteredFormasPago.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(filteredFormasPago.length / itemsPerPage);
        ...
        if (result.isConfirmed) {
            try {
                ...
                setFormasPago(formasPago.map(fp =>
                    fp.id === id ? { ...fp, estado: 'inactivo' } : fp
                ));
                await Swal.fire({
                    icon: 'success',
                    title: '¡Desactivado!',
                    text: 'Forma de pago desactivada exitosamente.',
    ...
    const handleReactivate = async (id: string) => {
        ...
        if (result.isConfirmed) {
            try {
                await updateFormaPago(id, { estado: 'activo' });
                setFormasPago(formasPago.map(fp =>
                    fp.id === id ? { ...fp, estado: 'activo' } : fp
                ));
                await Swal.fire({
                    icon: 'success',
                    title: '¡Reactivado!',
                    text: 'Forma de pago reactivada exitosamente.',
                    showConfirmButton: false,
                    timer: 1500
                });
            } catch (error) {
    ...
    const handleSave = async (formaPagoData: Omit<FormaPago, 'id'>) => {
        try {
            if (editingFormaPago) {
                await updateFormaPago(editingFormaPago.id, formaPagoData);
                await Swal.fire({
                    icon: 'success',
                    title: 'Actualizado',
                ...
                await createFormaPago(formaPagoData);
                await Swal.fire({
                    icon: 'success',
                    title: 'Creado',
    ...
    const openEditModal = (formaPago: FormaPago) => {
        setEditingFormaPago(formaPago);
        setShowModal(true);
    };

    const exportToExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(filteredFormasPago.map(fp => ({
            'Nombre': fp.forma_pago,
            'Estado': fp.estado
        })));
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "Formas de Pago");
        XLSX.writeFile(workbook, `FormasPago_${new Date().toISOString().split('T')[0]}.xlsx`);
    };
    ...
    const exportToPDF = () => {
        ...
        doc.text('Reporte de Formas de Pago', 14, 22);
        ...
        autoTable(doc, {
            head: [['Nombre', 'Estado']],
            body: filteredFormasPago.map(fp => [fp.forma_pago, fp.estado]),
            startY: 30,
    ...
    const manualSections: ManualSection[] = [
        {
            ...
            content: 'Aquí puede administrar las formas de pago del sistema (Efectivo, Tarjeta, etc.).'
        },
        {
            title: 'Crear Forma de Pago',
            content: 'Haga clic en "+ Nueva Forma de Pago", ingrese el nombre y guarde.'
        },
        {
    ...
    return (
        <div className="container mx-auto relative min-h-screen">
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 no-print">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Configura los métodos de pago aceptados</p>
                    </div>
                </div>
            ...
            <div className="mb-2 text-sm text-gray-500 dark:text-gray-400">
                Mostrando {filteredFormasPago.length === 0 ? 0 : indexOfFirstItem + 1} - {Math.min(indexOfLastItem, filteredFormasPago.length)} de {filteredFormasPago.length} registros
            </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                                ...
                                <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Forma de Pago</th>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {currentItems.map((fp, index) => (
                                <tr key={fp.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td className="p-4 text-gray-700 dark:text-gray-200 font-medium">{fp.forma_pago}</td>
                                    <td className="p-4">
                                        <span className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wide border ${fp.estado === 'activo'
            ...
            <ManualModal
                ...
                title="Manual de Formas de Pago"
                ...
                <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4 backdrop-blur-sm">
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100">
                        <FormaPagoForm
                            ...
                            onSubmit={handleSave}
                            onCancel={() => { setShowModal(false); setEditingFormaPago(null); }}
                        />
                    </div>
                </div>
            )}
