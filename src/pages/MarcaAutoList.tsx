...
import { getMarcasAuto, deleteMarcaAuto, reactivateMarcaAuto, createMarcaAuto, updateMarcaAuto } from '../services/marcaAutoService';
...
import { MarcaAutoForm } from '../components/marcaAuto/MarcaAutoForm';

const MarcaAutoList = () => {
    const navigate = useNavigate();
    const [marcas, setMarcas] = useState<MarcaAuto[]>([]);
    const [filteredMarcas, setFilteredMarcas] = useState<MarcaAuto[]>([]);
    const [loading, setLoading] = useState(true);
    const [searchTerm, setSearchTerm] = useState('');
    const [showModal, setShowModal] = useState(false);
    const [editingMarca, setEditingMarca] = useState<MarcaAuto | null>(null);
    ...
    useEffect(() => {
        ...
        const filtered = marcas.filter(m =>
            m.marca.toLowerCase().includes(lowerSearchTerm)
        );
    ...
    const fetchMarcas = async () => {
        try {
            setLoading(true);
            const { data } = await getMarcasAuto('', 1000, 0); // Fetch all for client-side filtering/pagination
            setMarcas(data);
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: `Error al cargar marcas: ${err.message || 'Error desconocido'}`
            });
        } finally {
            setLoading(false);
        }
        ...
        if (result.isConfirmed) {
            try {
                ...
                await Swal.fire({
                    icon: 'success',
                    title: '¡Desactivado!',
                    text: 'Marca desactivada exitosamente.',
        ...
        if (result.isConfirmed) {
            try {
                ...
                await Swal.fire({
                    icon: 'success',
                    title: '¡Reactivado!',
                    text: 'Marca reactivada exitosamente.',
                    showConfirmButton: false,
                    timer: 1500
                });
            } catch (error) {
    ...
    const openEditModal = (marca: MarcaAuto) => {
        setEditingMarca(marca);
        setShowModal(true);
    };

    const handleSave = async (data: Omit<MarcaAuto, 'id'>) => {
        try {
            if (editingMarca) {
                await updateMarcaAuto(editingMarca.id, data);
                await Swal.fire({
                    icon: 'success',
                    title: 'Actualizado',
    ...
    const exportToExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(filteredMarcas.map(m => ({
            'ID': m.id,
            'Marca': m.marca,
            'Estado': m.estado
        })));
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "Marcas de Autos");
        XLSX.writeFile(workbook, `MarcasAutos_${new Date().toISOString().split('T')[0]}.xlsx`);
    };
    ...
    const exportToPDF = () => {
        ...
        doc.text('Reporte de Marcas de Autos', 14, 22);
        doc.setLineWidth(0.5);
        doc.setDrawColor(52, 152, 219);
        doc.line(14, 25, 196, 25);
        autoTable(doc, {
            head: [['ID', 'Marca', 'Estado']],
            body: filteredMarcas.map(m => [m.id, m.marca, m.estado]),
            startY: 30,
            headStyles: { fillColor: [52, 152, 219] },
            alternateRowStyles: { fillColor: [240, 248, 255] },
        });
        doc.save(`MarcasAutos_${new Date().toISOString().split('T')[0]}.pdf`);
    };
    ...
    const manualSections: ManualSection[] = [
        {
            title: 'Gestión de Marcas de Autos',
            content: 'Aquí puede administrar las marcas de vehículos disponibles en el sistema.'
        },
        {
            title: 'Crear Marca',
            content: 'Haga clic en "+ Nueva Marca", ingrese el nombre (ej: Toyota, Nissan) y guarde.'
        },
        {
            title: 'Desactivar/Reactivar',
            content: 'Use los botones de acción para cambiar el estado de una marca. Las marcas inactivas no aparecerán en los selectores.'
        }
    ];
    ...
    return (
        <div className="container mx-auto relative min-h-screen">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 no-print">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Gestiona las marcas de vehículos disponibles</p>
                    </div>
                </div>
            ...
            <div className="mb-2 text-sm text-gray-500 dark:text-gray-400">
                Mostrando {filteredMarcas.length === 0 ? 0 : indexOfFirstItem + 1} - {Math.min(indexOfLastItem, filteredMarcas.length)} de {filteredMarcas.length} registros
            </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {currentItems.map((marca, index) => (
                                <tr key={marca.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    <td className="p-4 text-gray-500 dark:text-gray-400 font-mono text-sm">
                                        {indexOfFirstItem + index + 1}
                                    </td>
                                    <td className="p-4 text-gray-700 dark:text-gray-200 font-medium">{marca.marca}</td>
                                    <td className="p-4">
                                        <span className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wide border ${marca.estado === 'activo'
                                    ...
                                    <td className="p-4 flex gap-2 justify-end no-print">
                                        ...
                                        {marca.estado === 'activo' ? (
                                            ...
                                            <button
                                                onClick={() => handleReactivate(marca.id)}
                ...
                <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4 backdrop-blur-sm">
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100">
                        <MarcaAutoForm
                            ...
                            onSubmit={handleSave}
                            onCancel={() => { setShowModal(false); setEditingMarca(null); }}
                        />
                    </div>
                </div>
            )}
