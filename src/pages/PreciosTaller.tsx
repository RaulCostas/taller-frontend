...
import { getPreciosTaller, createPrecioTaller, updatePrecioTaller, deletePrecioTaller, reactivatePrecioTaller } from '../services/precioTallerService';
import { PrecioTallerForm } from '../components/taller/PrecioTallerForm';
import Pagination from '../components/Pagination';
import ManualModal, { type ManualSection } from '../components/ManualModal';
import { Tag } from 'lucide-react';

const PreciosTaller = () => {
    const [precios, setPrecios] = useState<PrecioTaller[]>([]);
    const [filteredPrecios, setFilteredPrecios] = useState<PrecioTaller[]>([]);
    const [loading, setLoading] = useState(true);
    const [searchTerm, setSearchTerm] = useState('');
    const [showModal, setShowModal] = useState(false);
    const [editingPrecio, setEditingPrecio] = useState<PrecioTaller | null>(null);
    ...
    const fetchPrecios = async () => {
        try {
            setLoading(true);
            const data = await getPreciosTaller();
            setPrecios(data);
    ...
    const handleSave = async (data: CreatePrecioTallerData) => {
        try {
            if (editingPrecio) {
                await updatePrecioTaller(editingPrecio.id, data);
                await Swal.fire({
                    icon: 'success',
                    title: 'Actualizado',
    ...
    const handleReactivate = async (id: string) => {
        ...
        if (result.isConfirmed) {
            try {
                await reactivatePrecioTaller(id);
    ...
    const openEditModal = (precio: PrecioTaller) => {
    ...
    const exportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "PreciosTaller");
        XLSX.writeFile(workbook, `PreciosTaller_${new Date().toISOString().split('T')[0]}.xlsx`);
    };
    ...
    const exportToPDF = () => {
        ...
        doc.text('Precios del Taller', 14, 22);
        ...
        autoTable(doc, {
            startY: 30,
            head: [['Categoría', 'Detalle', 'Precio', 'Moneda', 'Estado']],
        ...
        doc.save(`PreciosTaller_${new Date().toISOString().split('T')[0]}.pdf`);
    };
    ...
    const manualSections: ManualSection[] = [
        { title: 'Gestión de Precios', content: 'Administre los precios de servicios del taller por categoría.' },
        { title: 'Precios', content: 'Defina precios únicos para cada servicio del taller.' }
    ];
    ...
    return (
        <div className="container mx-auto relative min-h-screen">
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 no-print">
                <div className="flex items-center gap-4">
                    <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Defina precios de servicios del taller por categoría</p>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                                ...
                                <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Precio</th>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {currentItems.map((item, index) => (
                                <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td className="p-4 text-gray-700 dark:text-gray-200 font-medium">{item.categoria?.categoria_servicio || 'N/A'}</td>
                                    <td className="p-4 text-gray-700 dark:text-gray-200">{item.detalle}</td>
                                    <td className="p-4 text-gray-700 dark:text-gray-200">{item.precio}</td>
                                    <td className="p-4">
            ...
            <ManualModal
                ...
                title="Manual de Precios Taller"
