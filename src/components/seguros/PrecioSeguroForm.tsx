...
import type { CreatePrecioSeguroData, PrecioSeguro } from '../../types/precioSeguro';
...
import { Shield, List, FileText, DollarSign, Brush, Wrench, Save, X, Power } from 'lucide-react';

interface PrecioSeguroFormProps {
    initialData?: PrecioSeguro | null;
    onSubmit: (data: CreatePrecioSeguroData) => Promise<void>;
    onCancel: () => void;
}

export const PrecioSeguroForm = ({ initialData, onSubmit, onCancel }: PrecioSeguroFormProps) => {
    const [formData, setFormData] = useState<CreatePrecioSeguroData>({
        idseguro: initialData?.seguro?.id || '',
        idcategoria_servicio: initialData?.categoria?.id || '',
        detalle: initialData?.detalle || '',
        nivel1: initialData?.nivel1 || 0,
        nivel2: initialData?.nivel2 || 0,
        nivel3: initialData?.nivel3 || 0,
        pintado: initialData?.pintado || 0,
        instalacion: initialData?.instalacion || 0,
    ...
    useEffect(() => {
        const loadDependencias = async () => {
            try {
                const [segurosData, categoriasData] = await Promise.all([
                    getSeguros(),
                    getCategoriasServicio()
                ]);
                setSeguros(segurosData.filter(s => s.estado === 'activo'));
        ...
        <div className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div className="relative">
                            <Shield className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 dark:text-gray-400" size={18} />
                            <select
                ...
                <div className="grid grid-cols-3 gap-4">
                    <div>
                        <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">Nivel 1</label>
                        <div className="relative">
                    ...
                    <div>
                        <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">Nivel 2</label>
                        <div className="relative">
                            <DollarSign className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 dark:text-gray-400" size={18} />
                            <input
                                type="number"
                    ...
                    <div>
                        <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">Nivel 3</label>
                        <div className="relative">
                            <DollarSign className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 dark:text-gray-400" size={18} />
                            <input
                                type="number"
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">Pintado</label>
                        <div className="relative">
                            <Brush className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 dark:text-gray-400" size={18} />
                            <input
                                type="number"
                                step="0.01"
                                value={formData.pintado}
                                onChange={e => setFormData({ ...formData, pintado: parseFloat(e.target.value) })}
                                className="w-full pl-10 pr-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500"
                            />
                        </div>
                    </div>
                    <div>
                        <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">Instalación</label>
                        <div className="relative">
                            <Wrench className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 dark:text-gray-400" size={18} />
                            <input
                                type="number"
                                step="0.01"
                                value={formData.instalacion}
                                onChange={e => setFormData({ ...formData, instalacion: parseFloat(e.target.value) })}
                                className="w-full pl-10 pr-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500"
                            />
                        </div>
                    </div>
